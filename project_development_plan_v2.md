...
| 资产类型 | 推荐名称 | 备选方案 |
|---------|---------|---------|
| **PyPI 包名** | `deepseek-ocr-agent` | `visor-agent` |
| **GitHub 仓库** | `visor-agent/deepseek-ocr-agent` | - |
| **域名** | `visor-agent.com` | `deepseek-ocr.dev` |
| **品牌定位** | "Standard Vision Tool for AI Agents" | - |
...
| 任务 | 交付物 | 验收标准 | 技术要点 |
|------|--------|---------|---------|
| **1.1 注册项目资产** | - GitHub 仓库（public）<br>- PyPI 包名预留<br>- 域名注册 | `pip search deepseek-ocr-agent` 无冲突 | 使用 `twine` 上传占位包（v0.0.1） |
| **1.2 环境验证** | - 本地成功运行 HF 示例<br>- 测试 Tiny/Base/Gundam 3 种模式 | 能处理发票/合同/表格图像 | 测试设备：RTX 4090 + M2 Mac |
| **1.3 初始化项目结构** | 完整目录树（见下方） | `pytest --collect-only` 通过 | 包含占位符文件 |
| **1.4 依赖管理** | `requirements.txt` + `requirements-dev.txt` | `pip install -r requirements.txt` 成功 | 可选依赖用 `extras_require` |
| **1.5 设备检测模块** | `device_manager.py` | 通过单元测试 | 检测 CUDA/MPS/CPU + 内存估算 |
...
### 今天完成（Day 1）
- [ ] 检查 PyPI 包名可用性：访问 https://pypi.org 搜索 `deepseek-ocr-agent`
- [ ] 创建 GitHub 仓库：`visor-agent/deepseek-ocr-agent`（public）
- [ ] 注册域名（可选）：`visor-agent.com`
- [ ] 初始化项目结构：复制上述目录树到本地

### 本周完成（Day 1-7）
- [ ] 在本地成功运行 DeepSeek-OCR Tiny 模式
- [ ] 实现 `device_manager.py` 并通过单元测试
- [ ] 撰写 README.md 初版（包含愿景 + Roadmap）
- [ ] 上传占位包到 PyPI：`deepseek-ocr-agent==0.0.1`

### 下周计划（Day 8-14）
- [ ] 封装推理引擎 `infer.py`
- [ ] 实现自动降级装饰器
- [ ] 开发 Invoice Parser 初版
- [ ] 编写 LangChain 集成示例
